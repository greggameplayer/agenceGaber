...
def frameSeeAllTrips():
    ...
    indiceVoyage = 1
    for i in nb:
        buttonVoyage = tkinter.Button(frameSeeAllTrips, text= 'Voyage' + indiceVoyage, command = lambda id=i[0] : seeMore(id) , bg='grey15',fg='snow')
        buttonVoyage.pack()
        indiceVoyage += 1
...
def frameSeeMoreAboutTrip(id): 
    ...
    label4.pack()
    label5= tkinter.Label(frameSeeAllTrips, text = 'Nombre de places totales : {}'.format(info[2]), bg='grey15',fg='snow')
    label5.pack()
